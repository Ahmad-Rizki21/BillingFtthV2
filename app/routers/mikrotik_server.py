...
from ..schemas.mikrotik_server import MikrotikServer as MikrotikServerSchema, MikrotikServerCreate, MikrotikServerUpdate
...
async def update_mikrotik_server(
    server_id: int, server_update: MikrotikServerUpdate, db: AsyncSession = Depends(get_db)
):
    """
...
async def test_mikrotik_connection(server_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        ros_version = routerboard_info[0].get('current-firmware') if routerboard_info else 'N/A'
